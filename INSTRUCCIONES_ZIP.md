...
Se ha implementado la funcionalidad para **subir archivos ZIP junto con archivos EXE** en el gestor de versiones de ClubCheck. Ambos archivos son ahora obligatorios al subir una nueva versión.

### Características Implementadas

1. ✅ **Subida obligatoria de archivos ZIP y EXE**
   - Al subir una nueva versión, ahora se requieren ambos archivos
   - Validación automática de extensiones (.exe y .zip)
   - Backup automático de archivos existentes al reemplazar

2. ✅ **Ruta pública de descarga del ZIP**
   - URL: `/api/download-zip`
   - Sin restricciones de autenticación
   - Cualquier usuario puede descargar el archivo ZIP

3. ✅ **Base de datos actualizada**
   - Nuevos campos: `ZipUrl`, `ZipSha256`, `ZipFileSize`
   - Información completa de ambos archivos

4. ✅ **API actualizada**
   - Endpoint `/api/version`: Incluye información del ZIP
   - Endpoint `/api/check-update`: Incluye URLs y checksums del ZIP
   - Endpoint `/api/download-zip`: Descarga pública del ZIP
